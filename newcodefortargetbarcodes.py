...
def modified_save_excel():
    ...
    # Filter CSV and Reference Master DF based on Target Barcodes
    filtered_csv_df = csv_df[csv_df['BAR_CODE'].isin(target_barcodes_df['Target_Barcodes'])]
    filtered_master_df = reference_master_df[reference_master_df['Product ID'].isin(filtered_csv_df['PRD_NAME'])]
    
    # Proceed with matching and data processing
    merged_df = filtered_csv_df.merge(filtered_master_df, left_on='PRD_NAME', right_on='Product ID', how='left')
    missing_df = merged_df[merged_df['Product ID'].isna()]
    
    # Prepare final DataFrame with invoice details
